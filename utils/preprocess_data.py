...
project_path = '/home/quanmai/workspace/attention_braingnn'
root_path = os.path.join(project_path, 'data/')
data_path = os.path.join(root_path, 'ABIDE_pcp/cpac/filt_global')
phenotype = os.path.join(root_path, 'ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv')
# subjects_file = 'NYU.txt'
subjects_file = 'subject_ID.txt'
    ...
    for i in range(len(subject_IDs)):
        ...
        try:
            try:
                ...
                os.chdir(data_path + '/' + subject_IDs[i])
                filenames.append(glob.glob('*' + subject_IDs[i] + filemapping[file_type])[0])
        except IndexError:
...
def get_timeseries(subject_list, atlas_name, silence=False):
    ...
    for i in range(len(subject_list)):
        subject_path = os.path.join(data_path, subject_list[i])
        ro_file = [f for f in os.listdir(subject_path) if f.endswith('_rois_' + atlas_name + '.1D')]
        fl = os.path.join(subject_path, ro_file[0])
        if silence != True:
...
def subject_connectivity(timeseries, subjects, atlas_name, kind, iter_no='', seed=1234,
                         n_subjects='', save_path=data_path):
    """
...
def get_ids(num_subjects=None):
    subject_IDs = np.genfromtxt("{}/{}".format(data_path, subjects_file), dtype=np.dtype(str))
...
def get_networks(subject_list, kind, atlas_name="cc200", variable='connectivity', sparsify=False):
    """
    ...
    for subject in subject_list:
        ...
        fl = os.path.join(data_path, subject,
                              subject + "_" + atlas_name + "_" + kind.replace(' ', '_') + ".mat")


        matrix = sio.loadmat(fl)[variable]
        if sparsify:
            num_roi = matrix.shape[0]
            for i in range(num_roi):
                matrix[i] = sparsify_dense(np.abs(matrix[i]), sparse_factor)
...
def sparsify_dense(edge, k=0.1):
    numnodes = edge.shape[0]
    topk = int(k*numnodes)

    ret = np.zeros_like(edge)
    sort_idx = np.argsort(edge)
...
def site_encode(subj_list, score, binary=False):
    # sitelst= sorted(list(set(labels)))
    class_type = get_subject_score(subj_list, score)
    classes = []
    num_subjects = len(subj_list)
    for i in range (num_subjects):
        if class_type[subj_list[i]] not in classes:
            classes.append(class_type[subj_list[i]])
    classes.sort()
    classes_onehot = {c: np.identity(len(classes))[i, :] for i, c in enumerate(classes)}
    # labels_onehot = np.array(list(map(classes_dict.get, classes)), dtype=np.int32)
    if binary:
        classes_dict = {j: 0 for _, j in enumerate(classes)}
        classes_dict[classes[0]] = 1
    else:    
        classes_dict = {j: i for i, j in enumerate(classes)}
    return classes_onehot, classes_dict
...
if __name__ == "__main__":
    ...
    filename = ['50346']
    get_networks(filename, kind='partial correlation')